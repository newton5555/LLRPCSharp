...
internal static class Llrp20TagAccessCompiler
{
        ...
        bool useBlockWrite = false)
    {
        ...
        var target = new C1G2TargetTag((byte)selection.MemoryBank, selection.Match, selection.BitPointer, LlrpWireBits.ToBits(selection.Mask.Span, selection.BitLength), LlrpWireBits.ToBits(selection.Data.Span, selection.BitLength));
        var command = new AccessCommand(new C1G2TagSpec([target]), opSpecs, []);
        return new AccessSpec(accessSpecId, first.AntennaId, V20Enumerations.AirProtocols.EPCGlobalClass1Gen2, V20Enumerations.AccessSpecState.Disabled, roSpecId,
            new AccessSpecStopTrigger(V20Enumerations.AccessSpecStopTriggerType.Operation_Count, 1), command,
            new AccessReportSpec(V20Enumerations.AccessReportTriggerType.End_Of_AccessSpec), []);
    }
    ...
    private static C1G2Lock CompileLock(LockTagRequest lockReq, ushort opSpecId, uint accessPassword)
    {
        ...
        AddPayloadIfSet(payloads, V20Enumerations.C1G2LockDataField.Kill_Password, lockReq.KillPasswordLockMode);
        AddPayloadIfSet(payloads, V20Enumerations.C1G2LockDataField.Access_Password, lockReq.AccessPasswordLockMode);
        AddPayloadIfSet(payloads, V20Enumerations.C1G2LockDataField.EPC_Memory, lockReq.EpcMemoryLockMode);
        AddPayloadIfSet(payloads, V20Enumerations.C1G2LockDataField.TID_Memory, lockReq.TidMemoryLockMode);
        AddPayloadIfSet(payloads, V20Enumerations.C1G2LockDataField.User_Memory, lockReq.UserMemoryLockMode);
    ...
    private static void AddPayloadIfSet(List<C1G2LockPayload> payloads, V20Enumerations.C1G2LockDataField field, TagLockMode mode)
    {
        if (mode == TagLockMode.NoChange)
        {
            return;
        }
        V20Enumerations.C1G2LockPrivilege privilege = mode switch
        {
            TagLockMode.Accessible => V20Enumerations.C1G2LockPrivilege.Read_Write,
            TagLockMode.AlwaysAccessible => V20Enumerations.C1G2LockPrivilege.Perma_Unlock,
            TagLockMode.SecuredWrite => V20Enumerations.C1G2LockPrivilege.Unlock,
            TagLockMode.AlwaysNotWritable => V20Enumerations.C1G2LockPrivilege.Perma_Lock,
