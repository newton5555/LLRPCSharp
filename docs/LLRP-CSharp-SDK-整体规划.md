...
module ID 和 Codec wire identity 冲突都会在创建 Reader 时失败。项目内的 Impinj XML（Vendor ID 25882）作为本地
生成输入并保持 Git 忽略；其 4 条 Custom Message、95 个 Custom Parameter、42 个枚举已生成到受 Git 跟踪的
`LlrpSdk.Extensions.Impinj`。该模块以 `--dependency-root-namespace LlrpNet.Protocol` 引用标准模型，并由
