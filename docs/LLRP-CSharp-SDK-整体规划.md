...
M6 的目标是让 `ConnectAsync()` 内部完成共同版本选择、Registry/Adapter 切换、Capabilities/Identity 查询和扩展激活，业务代码不自行处理协商。默认 `LlrpProtocolVersionPolicy.Auto` 实现 1.0.1 ↔ 1.1 协商：读写器返回 `M_UnsupportedVersion` 时保留 1.0.1，否则成功执行 `SET_PROTOCOL_VERSION(2)` 后切换到 1.1。对收到高版本协商报文即断线的旧设备，调用方可选择 `Force101` 跳过探测；`Force11` 则要求协商成功切换到 1.1。Reader Event、配置初始化和 2.0 仍是后续 M6 工作。
...
截至 2026-07-24：M5a（标准 XML 生成链）和 M5b（定义/生成工作流）均已完成。LTK XML Importer、YAML Loader、ProtocolModel、Validator、C# / Codec / Registry Module Generator 与独立命令行工具均已实现；标准 1.0.1 的 42 个消息、111 个参数、42 个枚举和 14 个 Choice 已物化为受 Git 跟踪的 `.g.cs` 源码。生成器已覆盖参数引用、Choice、Custom 类型、长度计算、枚举值验证与依赖定义；YAML schema、`--verify` 和可重复执行的扩展定义命令已纳入仓库。1.1 已作为相对于 1.0.1 的受核验 YAML Delta 合成并生成受 Git 跟踪的模型；2.0 Delta 仍属于 M6 后续输入。

### M6：LLRP 1.1 与 2.0

1.1 定义和适配器、2.0 Delta 与适配器、版本协商互操作，随后扩展 Gen2v2 能力。

截至 2026-07-24：`ILlrpProtocolAdapter` 已接管标准 Codec 注册、盘点编译、报告翻译，以及 ROSpec/AccessSpec 的请求与响应映射。1.0.1 与 1.1 Adapter 已在同一 `RoSpecs` / `AccessSpecs` 服务入口切换；`ConnectAsync()` 使用 1.1 的 `GET_SUPPORTED_VERSION` / `SET_PROTOCOL_VERSION` 协商，在旧读写器明确返回 `M_UnsupportedVersion` 时回退 1.0.1。`LlrpProtocolVersionPolicy.Force101` 可跳过该探测以兼容激进断连设备，`Force11` 则拒绝降级。CLI 支持 `--llrp auto|1.0.1|1.1`。2.0 Delta、2.0 Adapter 与实机协商互操作仍待完成。
...
module ID 和 Codec wire identity 冲突都会在创建 Reader 时失败。项目内的 Impinj XML（Vendor ID 25882）以 LTK
Definition Files 10.58.0 作为本地生成输入并保持 Git 忽略；其 4 条 Custom Message、104 个 Custom Parameter、49 个
枚举已生成到受 Git 跟踪的 `LlrpSdk.Extensions.Impinj`。该模块以 `--dependency-root-namespace LlrpNet.Protocol` 引用标准模型，并由
`builder.UseImpinj()` 在连接前注册 Codec；`ImpinjReaderExtension` 只在协商到 LLRP 1.0.1 且 Manufacturer ID
为 25882 时激活。`IReaderExtension`、`UseReaderExtension(...)` 和 `reader.Extensions` 已建立第二阶段生命周期：
标准初始化后按 Manufacturer/Model/Firmware/协议版本匹配，重复 extension ID 在构建 Reader 时失败，同一非空互斥组
的多个匹配项会拒绝连接而不会任意选择。厂商命令、Settings/TagReport Contributor 及 Impinj 的真实型号/固件规则仍待
真机样本完成。
...
诊断基线还提供 `LlrpFrameJournal`：它实现既有的 `ILlrpFrameObserver`，有界地复制并按时间顺序保留精确收发帧及连接代次；应用可通过 `LlrpReaderBuilder.WithFrameObserver(...)` 注入，不影响高级 SDK API 的版本隔离。

`LlrpReaderBuilder.WithAutomaticReconnect(...)` 提供默认关闭的有限指数退避重连。它只在意外断线后恢复连接和标准能力初始化；ROSpec、AccessSpec 与托管盘点不会被静默重建，仍需后续的显式恢复/协调策略处理。
