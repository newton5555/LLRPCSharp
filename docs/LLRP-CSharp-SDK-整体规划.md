...
M6 的目标是让 `ConnectAsync()` 内部完成共同版本选择、Registry/Adapter 切换、Capabilities/Identity 查询和扩展激活，业务代码不自行处理协商。当前已实现 1.0.1 ↔ 1.1 协商：读写器返回 `M_UnsupportedVersion` 时保留 1.0.1，否则成功执行 `SET_PROTOCOL_VERSION(2)` 后切换到 1.1；Reader Event、配置初始化和 2.0 仍是后续 M6 工作。
...
截至 2026-07-24：`ILlrpProtocolAdapter` 已接管标准 Codec 注册、盘点编译、报告翻译，以及 ROSpec/AccessSpec 的请求与响应映射。1.0.1 与 1.1 Adapter 已在同一 `RoSpecs` / `AccessSpecs` 服务入口切换；`ConnectAsync()` 使用 1.1 的 `GET_SUPPORTED_VERSION` / `SET_PROTOCOL_VERSION` 协商，在旧读写器明确返回 `M_UnsupportedVersion` 时回退 1.0.1。2.0 Delta、2.0 Adapter 与实机协商互操作仍待完成。
