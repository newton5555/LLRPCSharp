...
生成物或发布 Impinj 扩展。`IReaderExtension`、`UseReaderExtension(...)` 和 `reader.Extensions` 已建立第二阶段
生命周期：标准初始化后按 Manufacturer/Model/Firmware/协议版本匹配，重复 extension ID 在构建 Reader 时失败，
同一非空互斥组的多个匹配项会拒绝连接而不会任意选择。Reader Extension 的厂商命令、Settings/TagReport Contributor
及 Impinj 的真实型号/固件规则仍待许可范围和真机样本一并完成。
