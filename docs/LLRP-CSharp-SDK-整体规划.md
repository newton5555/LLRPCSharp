...
await reader.Protocol.TransactAsync<AddRoSpecResponse>(request);
...
与具体 LLRP 版本和厂商无关，负责可靠通信基础设施：TCP 生命周期、半包/粘包和帧切分、公共 Message Header、Message ID、请求/响应事务匹配、超时和取消、Keepalive、原始帧诊断、Registry 入口及 Unknown/Raw 类型保留。
...
`ConnectAsync()` 内部完成连接、Reader Event、共同版本选择、Registry 切换、Capabilities/Identity/Configuration 查询和扩展激活，业务代码不自行处理协商。
...
截至 2026-07-23：M1 已完成。Header、Buffer/Bit Reader/Writer、TV/TLV Header、流式 Frame Decoder、Message ID、Transaction、TCP Transport、Session、Registry、Unknown/Raw 与 Custom 注册均已实现。底层采用 `Microsoft.Extensions.Logging` 抽象，并通过 `ILlrpFrameObserver` 保留完整收发帧观测；即使应用只使用 `LlrpReader` 高级入口，底层报文日志/抓包仍可由 Builder 注入并持续工作。边界和内存所有权见 ADR 0001。Core 当前有 92 项自动化测试通过，Protocol 当前有 87 项自动化测试通过。

### M2：`LlrpReader` 会话框架

Builder、连接生命周期、状态机、版本协商基础、Capabilities、Identity、Protocol 入口和扩展注册容器。

截至 2026-07-23：Builder、不可变 Options、连接状态机、Raw/Typed Protocol 入口、自定义 Registry 回调、Keepalive 自动应答、断线传播与重连基础已实现。当前正在接通 1.0.1 `GetReaderCapabilities` 初始化，使 `Ready` 严格表示能力查询成功；完整版本协商和扩展激活仍在后续批次。
...
截至 2026-07-23：Core LTK XML Importer 与 ProtocolModel/Validator 基础已实现，可导入 42 个标准消息、111 个标准参数、42 个枚举和 14 个 Choice；Custom 定义导入、组合校验与长度/循环分析正在补齐。Generator、YAML Loader 和生成代码接入尚未完成。
