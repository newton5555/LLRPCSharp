...
│  • 可选的 1.0.1 Impinj 厂商扩展输入 (Impinjdef.xml；本地且待授权)       │
...
### 场景 1：使用标准 1.0.1 / 1.1 读写器
- **客户需要准备什么**：**无需准备任何文件**。
- **项目处理机制**：
  - 1.0.1 标准已基于内置 XML 预编译。
...
```text
[ 固定 XML 基线 ] (1.0.1 标准；Impinj 为本地待授权的可选输入)
         │
         ▼ (解析为内存 1.0.1 基础模型)
...
| **Impinj 1.0.1 设备** | **本地 XML（待授权）** | 厂商授权后由本项目生成扩展 DLL | 暂不适用；在获得提交与再分发授权前不发布 |
