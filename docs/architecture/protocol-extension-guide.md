...
> **核心原则**：**XML 为固定历史基线；SDK 内置支持 1.0.1 / 1.1 / 2.0；客户仅需为“厂商扩展”及“未来新协议”编写 YAML 增量。**
...
│  • 仅包含 1.0.1 Impinj 厂商扩展定义 (Impinjdef.xml)                    │
...
### 场景 1：使用标准 1.0.1 / 1.1 / 2.0 读写器 或 Impinj 1.0.1 读写器
- **客户需要准备什么**：**无需准备任何文件**。
- **项目处理机制**：
  - 1.0.1 标准与 Impinj 1.0.1 扩展已基于内置 XML 预编译。
  - LLRP 1.1 与 LLRP 2.0 属于本 SDK 内置范围，**本 SDK 会直接内置提供**对应的 `llrp-1.1.yaml` 与 `llrp-2.0-delta.yaml` 并自动生成代码，客户开箱即用。
...
|---|---|---|---|
| **1.0.1 标准 / Impinj 1.0.1 设备** | **XML** | 本项目提供（已编译至 DLL） | ❌ **否**（运行时无需文件，开箱即用） |
| **LLRP 1.1 / LLRP 2.0 标准** | **YAML (差量)** | 本 SDK 提供（编译至 DLL） | ❌ **否**（SDK 内置完成，开箱即用） |
