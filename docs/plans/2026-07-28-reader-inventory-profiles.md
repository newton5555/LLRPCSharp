...
| Application settings | `InventorySettings` | Explicit business overrides for one inventory operation. |
...
3. **M3 — merge model:** represent per-field provenance and ensure `InventorySettings` overrides profile recommendations without making unspecified fields look explicit.
...
- Do not deserialize arbitrary `InventorySettings.Extensions` as a profile format.
