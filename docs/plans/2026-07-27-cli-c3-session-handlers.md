...
Keep inventory lifecycle ownership, connection checks for start/stop/status, task cancellation, `InventorySettings.AntennaIds`, and tag-line rendering. The later Reader-first plan intentionally replaces the old positional start syntax with a session-local `DesiredInventorySettings` draft and `inventory start [--antennas <id,id|all>]`.
