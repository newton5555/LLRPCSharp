...
| 日期 | 设备 | 结果 |
| 2026-08-31 | Seuic UF40 `192.168.41.148`，LLRP 1.0.1，Manufacturer `57690`、Model `40`、Firmware `1.0.0.233`，1 天线 | 按发布门槛执行 `dotnet run --project tools/LlrpSdk.LiveSmoke -- 192.168.41.148 --inventory`；TCP 连接建立后设备在 Auto 版本协商阶段关闭连接（收到响应后出现 EOF），命令退出码 1。未创建临时 ROSpec/AccessSpec，无需清理；未写标签或设备配置。随后按设备已知能力执行 `dotnet run --project src/LlrpCli -- inventory 192.168.41.148 --llrp 1.0.1 --vendor seuic --duration 10 --output json --yes`；强制 1.0.1 盘点成功，10 秒收到 16 个唯一标签，全部来自天线 1，峰值 RSSI -35 至 -71，输出确认 `managedInventoryCleared=true`、`readerConfigurationRetained=true`；未写标签或设备配置。 |
| 2026-08-27 | Seuic UF40 `192.168.41.148`，LLRP 1.0.1，Manufacturer `57690`、Model `40`、Firmware `1.0.0.233`，1 天线 | 按发布门槛执行 `dotnet run --project tools/LlrpSdk.LiveSmoke -- 192.168.41.148 --inventory`；TCP 端口可达，但设备在 Auto 版本协商阶段关闭连接（收到 `GET_SUPPORTED_VERSION_RESPONSE` 后连接结束），命令退出码 1。未创建临时 ROSpec/AccessSpec，无需清理；未写标签或设备配置。随后按设备已知能力以 `dotnet run --project src/LlrpCli -- inventory 192.168.41.148 --llrp 1.0.1 --vendor seuic --duration 10 --output json --yes` 完成强制 1.0.1 盘点：10 秒收到 19 个唯一标签，全部来自天线 1，峰值 RSSI -33 至 -76，`managedInventoryCleared=true`、`readerConfigurationRetained=true`；未写标签或设备配置。 |
