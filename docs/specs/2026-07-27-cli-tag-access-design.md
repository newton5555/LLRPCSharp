...
**Goal:** Provide safe Live Shell paths for standard C1G2 tag access while preserving explicit confirmation for destructive operations.
...
- `tag write` accepts the same target information plus words. Without `--yes` it renders the derived request only; with `--yes` it calls `WriteTagMemoryAsync` on the current session Reader.
- `tag sequence` exposes multiple standard operations in one temporary AccessSpec. Non-read operations require `--yes`.
- Live Shell reuses its current connection; no one-shot Reader lifecycle is provided.
...
- Writes require an explicit `--yes`; the default path remains a session-local preview and does not write tag memory.
...
Tag operations are Live Shell commands. Establish the connection first with `connect <HOST> [port] [--llrp ...] [--vendor ...]`, then run the same `tag` subcommands in that session.
...
- An AccessSpec/OpSpec failure renders the standard error returned by the SDK and leaves the Live Shell session available for diagnosis.
...
- Vendor-specific access commands.
